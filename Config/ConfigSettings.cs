...
namespace ReservedBoomboxSlot.Config
{
    public static class ConfigSettings
    {
        //public static ConfigEntry<bool> hideBoomboxMeshShoulder;
        ...
        public static void BindConfigSettings()
        {
            ...
            //hideBoomboxMeshShoulder = AddConfigEntry(Plugin.instance.Config.Bind("Client-side", "HideBoomboxOnShoulder", false, "Hides the Boombox mesh while on your shoulder. Only applies in scenarios where you can view your player in third person."));
            overrideItemSlotPriority = AddConfigEntry(Plugin.instance.Config.Bind("Server-side", "BoomboxSlotPriorityOverride", 21, "[Host only] Manually set the priority for this item slot. Higher priority slots will come first in the reserved item slots, which will appear below the other slots. Negative priority items will appear on the left side of the screen, this is disabled in the core mod's config."));
            overridePurchasePrice = AddConfigEntry(Plugin.instance.Config.Bind("Server-side", "BoomboxSlotPriceOverride", 0, "[Host only] Manually set the price for this item in the store. Setting 0 will force this item to be unlocked immediately after the game starts."));
